...
def build_portaudio():
    ...
    if not os.path.exists(portaudio_path):
        st.error("PortAudio directory not found. Ensure the 'portaudio-v19' folder is in the correct location.")
        return
    try:
        os.system(f"cd {portaudio_path} && ./configure && make && sudo make install")
    except Exception as e:
        st.error(f"Failed to build PortAudio: {e}")

# Call the build_portaudio function before using PyAudio
build_portaudio()

# Record audio to text using PyAudio
